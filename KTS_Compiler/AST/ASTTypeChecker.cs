...
namespace KTS_Compiler
{
    public class ASTTypeChecker : Expr.IVisitor<TypeSpecifier>, Stmt.IVisitor<bool>
    {
        ...
        public ASTTypeChecker(List<Stmt> functionDeclarations)
        {
            ...
            FunctionTable.Add("printf", new FunctionSymbol
            {
        ...
        public TypeSpecifier VisitArrayInitializer(ArrayInitializer exp)
        {
            foreach (var expr in exp.DimensionSizes)
            {
                ...
                if (!ts.IsInt() || expr is not Expr.Literal)
                {
                    Console.WriteLine($"Array initializer dimensions must be constant integer expressions");
        ...
        public TypeSpecifier VisitInvokeExpression(Invoke exp)
        {
            ...
            else
            {
                ...
                if (function.VarArgs)
                {
                    if (function.Parameters.Count > exp.Arguments.Count)
                    {
        ...
        {
            if (exp.Type == TypeEnum.STRING)
            {
                return new TypeSpecifier { Type = TypeEnum.I8, Dimensions = 1 };
            }
