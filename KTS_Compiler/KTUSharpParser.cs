...
namespace KTS_Compiler
{
    public class KTUSharpParser
    {
        ...
        private TypeSpecifier TypeSpec(string error)
        {
            ...
            var typeSpec = new TypeSpecifier { Dimensions = 0, Type = type };
        ...
        private Expr Multiplicative()
        {
            ...
            while (Match(TokenType.DIV, TokenType.MULT, TokenType.PERCENT))
            {
                Token op = Previous;
                Expr right = Cast();
                expr = new Expr.Binary(expr, op, right);
        ...
        private Expr Primary()
        {
            ...
            if (Match(TokenType.STRING)) return new Expr.Literal(TypeEnum.STRING, Previous.Value);
